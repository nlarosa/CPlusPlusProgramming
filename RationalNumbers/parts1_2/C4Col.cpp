...
C4Col C4Col::operator+=( char newDisc )	// overrides the += operator to "add" a disc, member function because left side of operator is our object
{
	addDisc( newDisc );		// calls the addDisc function to put a new disc in the specified column
	return (*this);			// returns the actual column that was modified (dereferenced the pointer to class instance)	
}
...
char C4Col::getDisc( int discIndex ) const	// returns the character (X or O) at the specified column index
{
...
int C4Col::getMaxDiscs() const			// returns the maximum number of discs in a column, determined by constructor (get function)
{
	return maxDiscs;
}

int C4Col::getNumDiscs() const			// returns the current number of discs in a column
{
	return numDiscs;
}

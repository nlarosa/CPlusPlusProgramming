...
ostream &operator<<( ostream &printer, const C4Board &ourBoard )		// overloading the cout stream to allow us to display the board with cout
{										// Note: the body of this code is originally from the display() function
	int rowNum;								// with all the cout calls replaced with "printer" (ostream pointer name)
	int colNum;
	...
	for( colNum = 0; colNum < ourBoard.numColumns; colNum++ )				// print column labels	
	{
		if( count == 0 )		
		{
			printer << "  " << colNum;
		...
		else if( previous < 10 )	printer << "   " << colNum;
		else				printer << "  " << colNum;		
	...
	for( rowNum = (ourBoard.Board[0].getMaxDiscs()-1); rowNum >= 0; rowNum-- )	// finds how many discs are in a column and increments down (height of board)
	{
		printer << "\t";		

		for( colNum = 0; colNum < ourBoard.numColumns; colNum++ )
		{
			printer << "| " << ourBoard.Board[colNum].getDisc(rowNum) << " ";
		}
	...
	return printer;					// returns the ostream, which must print to the screen
}
...
void C4Board::play()
{
	...
	cout << *this;					// send the current object instance to the overloaded ostream

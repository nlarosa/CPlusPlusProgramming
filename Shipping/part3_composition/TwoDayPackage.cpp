...
TwoDayPackage::TwoDayPackage( const string &sName, const string &sAddress, const string &sCity, const string &sState, const string &sZip, const string &rName, const string &rAddress, const string &rCity, const string &rState, const string &rZip, double weight )
	: memPackage( sName, sAddress, sCity, sState, sZip, rName, rAddress, rCity, rState, rZip, weight )	// member initialization syntax to construct data member
{
...
{
	return memPackage.calculateCost() + flatFee;	// this value is guarenteed to be non-negative
}

void TwoDayPackage::print()			// prints the sender and recipient information, in addition to total cost
{
	cout << endl;
